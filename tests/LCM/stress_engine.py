...
async def stress_loop(engine, cmpl, state):
    ...
    while state["running"]:
        ...
        # Ingest User Message
        engine.process_and_save_message(session_id, MessageRole.USER, [{"text": p}])
        ...
        # Ingest AI Response
        ai_msg = engine.process_and_save_message(session_id, MessageRole.ASSISTANT, [{"text": ai_text}])
